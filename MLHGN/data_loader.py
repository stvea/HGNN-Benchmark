...
class data_loader:
    def __init__(self, path):
        ...
        self.edge_labels_train = self.load_edge_labels('link.label.dat')
        self.edge_labels_test = self.load_edge_labels('link.label.test.dat')
    ...
    def load_edge_labels(self, name):
        ...
        with open(os.path.join(self.path, name), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                head_node_id, tail_node_id, edge_type = int(th[0]), int(th[1]), int(th[2])
                head_node_idx.append(head_node_id)
                tail_node_idx.append(tail_node_id)
                label.append(edge_type)
                num_classes = num_classes if num_classes>edge_type else edge_type
                count+=1
        if len(head_node_idx) == len(tail_node_idx) == len(label):
            return {'head_node_idx':head_node_idx,'tail_node_idx':tail_node_idx,"label":label,"count":count,"num_classes":num_classes+1}
        else:
