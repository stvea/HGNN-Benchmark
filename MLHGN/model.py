...
class HGT(nn.Module):
    def __init__(self, G, n_inps, n_hid, n_out, n_layers, n_heads, use_norm=True):
        ...
        self.adapt_layer = nn.Linear(n_layers*n_hid,n_hid)
        # self.out = nn.Sequential(nn.Dropout(0),nn.Linear(n_hid*2,n_hid),nn.ReLU(inplace=True),nn.Dropout(0),nn.Linear(n_hid,n_out))
        self.out = nn.Sequential(nn.Dropout(0),nn.Linear(n_hid*2,n_hid*3),nn.Dropout(0),nn.Linear(n_hid*3,n_out))
        # self.out_f1 = nn.Linear(n_hid*2, n_hid*3)
        # self.out_f2 = nn.Linear(n_hid*3, n_out)

    def forward(self, G, left_embedding_idx, right_embedding_idx):
        if True:
            for ntype in G.ntypes:
            ...
            for ntype in G.ntypes:
                n_id = G.node_dict[ntype]
                G.nodes[ntype].data['h0'] = torch.tanh(self.adapt_ws[n_id](G.nodes[ntype].data['inp']))
            for i in range(self.n_layers):
                self.gcs[i](G, f'h{i}', f'h{i+1}')

        embedding = torch.cat(tuple([G.nodes[ntype].data['h'] for ntype in G.ntypes]),0)

        left_embedding = embedding.index_select(0, left_embedding_idx)
        right_embedding = embedding.index_select(0, right_embedding_idx)

        # return self.out(G.nodes[out_key].data['h'])
        return self.out(torch.cat((left_embedding,right_embedding),1))
        # return self.out_f2(self.out_f1(torch.cat((left_embedding,right_embedding),1)))
